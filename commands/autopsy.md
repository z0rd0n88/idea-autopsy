...
6. Invoke the chosen skill by its registered/namespaced name; if needed read its bundled SKILL.md and follow it through the host's skill mechanism. Pass source/report IDs, original request, approved context, slug, permitted flags and any explicit per-role caller model requirement exactly as supplied. Do not turn an absent model policy into a default. One brief routing line is sufficient.
