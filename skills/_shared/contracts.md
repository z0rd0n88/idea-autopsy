...
After an allowed dispatch, obtain the actual model from a host-exposed record, or use `unknown`. Call `model_observation` with the preflight result and `actual_model`; supply `host_resolved_model` for an alias only when the host exposes that resolution. Do not infer an alias's version or trust a reviewer's self-description as host provenance. Record one observation, including a blocked preflight with actual model `unknown`, per role in `result.review_protocol.roles`. Without a caller policy, dispatch normally and still record the actual/exposed model or `unknown`. An unconfirmed mandatory role makes its review coverage incomplete; evaluation passes the same `review_protocol` to `policy.py verdict`, which returns `incomplete_coverage` and a null verdict. Stress tests and strategies must name the missing role and qualify any conclusion that depended on it.
...
| `verdict` | `findings`, `coverage: {axis: status}`, `decision_evidence: {sufficient, rationale, sources}`, optional `review_protocol`, `reusable_assets`, `severity_resolutions`, `independence_evidence`, `override`; returns assessment status, mechanical/reported verdict, model-policy gaps and audit details. |
