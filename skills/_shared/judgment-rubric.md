...
Store `result.review_protocol` with `protocol_id: judgment-rubric-v1`, actual source hash, decision/evidence references, prompt identifier or hash, exposed model/version per role (or `unknown`), actual blinding conditions, unavailable reviewers, and limitations. Store `result.judgment_audit`, `result.forecast_checks`, and `result.success_case` for review/strategy reports; store `result.revision_changes` for revisions. These are additive report metadata, not new verdict inputs. Do not fill missing provenance by guessing.
