...
4. With `--validate`, add the validator in `strategy` mode to `expected_model_roles` before dispatch, apply the same model-policy preflight and observation, and authorize only the selected query material. It returns claim-level sources and limits; it never writes shared state. Without the flag, external claims remain unchecked. Competitors can demonstrate demand or switching challenges; presence alone does not disqualify an option.
