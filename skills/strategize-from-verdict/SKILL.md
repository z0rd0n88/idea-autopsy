...
7. Save an immutable `strategy` report with ranked options, evidence, finding mappings and pending decisions. Once the user selects a path, provide that decision and findings to `iterate-to-v2`, which writes the next proposal automatically. In a loop, respect the same version cap and resume point.
