...
3. Dispatch A, B and C concurrently with the brief and their lens below. Use fresh reviewer contexts; they must not see each other's outputs. Explicitly prohibit network access, file/state changes, further agents and outside actions. If a reviewer is unavailable, record incomplete coverage and continue useful analysis without pretending it passed.
4. Verify every Critical/High and any finding that could drive an action using a fresh verifier and the shared procedure. If `--verify-claims` is authorized, use the validator in `claims` mode with the approved query material. Integrate new findings through the same verification process.
...
6. Write an immutable `stress_test` report via `state.py report`, including structured final findings and coverage in `result`. Refresh the active findings view with a generation-checked update. Return findings and the next action: `iterate-to-v2` writes the revision, asking only about consequential decisions and unavailable facts.
